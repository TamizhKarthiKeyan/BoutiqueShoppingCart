...
const OktaLoginPage = () => {
  ...
  const handleOktaLogin = async () => {
    ...
    try {
      ...
      const originalUri = window.location.origin + '/profile';
  ...
  const handleOktaSignup = async () => {
    ...
    try {
      ...
      // Note: You need to configure self-registration in your Okta tenant
      const originalUri = window.location.origin + '/profile';
      await oktaAuth.setOriginalUri(originalUri);
      
      // Append the signup parameter to redirect to registration
      await oktaAuth.signInWithRedirect({
